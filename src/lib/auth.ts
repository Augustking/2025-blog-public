...
import { requireWriteApproval } from './write-gate'
...
export async function getAuthToken(): Promise<string> {
	// 0. 写权限门禁：先确认服务端口令会话已解锁（未解锁会弹密码框）
	const approved = await requireWriteApproval('进行写操作前需要验证身份')
	if (!approved) {
		throw new Error('未解锁写作权限')
	}
